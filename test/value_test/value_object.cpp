...
// --- Obj Construction and Type Checks ---
M_TEST(Value, Obj) {
    // Default and jsonlicit construction
    M_ASSERT_NO_THROW(Json obj_default{Json::Obj{}});
    M_ASSERT_EQ(Json{Json::Obj{}}.type(), json::Type::eObj);
    M_ASSERT_EQ(Json{Json::Obj{}}.obj().size(), 0);
    M_ASSERT_TRUE(Json{Json::Obj{}}.to<Json::Obj>().empty());

    M_ASSERT_NO_THROW(Json obj_jsonlicit{Json::Obj{}});
    M_ASSERT_NO_THROW(Json obj_init{Json::Obj{{"key1", 1}, {"key2", 2}}});

    // Direct construction and mixed types
    Json obj_direct{Json::Obj{{"name", "John"}, {"age", 30}, {"active", true}}};
    M_ASSERT_EQ(obj_direct.obj().size(), 3);

    Json mixed_obj{Json::Obj{
        {"number", Json::Num(42)},
        {"string", "hello"},
        {"boolean", true},
        {"null_value", nullptr}
    ...
    // Type checking
    Json obj_val{Json::Obj{{"a", 1}, {"b", 2}, {"c", 3}}};
    Json empty_obj{Json::Obj{}};
    M_ASSERT_TRUE(obj_val.is_obj());
    M_ASSERT_FALSE(obj_val.is_str());
    M_EXPECT_STREQ(json::type_name(obj_val.type()), "Obj");

    // --- Size and Empty Checks ---
    Json size_test{Json::Obj{{"a", 1}, {"b", 2}, {"c", 3}, {"d", 4}, {"e", 5}}};
    M_ASSERT_EQ(size_test.obj().size(), 5);
    Json empty_size_test{Json::Obj{}};
    M_ASSERT_EQ(empty_size_test.obj().size(), 0);
    M_ASSERT_TRUE(empty_size_test.to<Json::Obj>().empty());

    // --- Key Access and Modification ---
    Json subscript_test{Json::Obj{{"first", 10}, {"second", 20}, {"third", 30}}};
    M_ASSERT_EQ(subscript_test["first"].to<Json::Num>(), 10);

    Json mixed_subscript{Json::Obj{
        {"number", 42},
        {"text", "test"},
        {"flag", true},
        {"empty", nullptr}
    ...
    Json modify_test{Json::Obj{{"a", 1}, {"b", 2}, {"c", 3}}};
    modify_test["a"] = 100;
    ...
    // Add new key
    Json new_key_test{Json::Obj{{"existing", 1}}};
    new_key_test["new_key"] = "new_value";
    M_ASSERT_EQ(new_key_test.obj().size(), 2);

    // --- Const Key Access ---
    const Json const_obj{Json::Obj{{"x", 1}, {"y", 2}, {"z", 3}}};
    M_ASSERT_EQ(const_obj["y"].to<Json::Num>(), 2);

    // --- Safe Access with at() ---
    Json at_test{Json::Obj{{"alpha", 100}, {"beta", 200}, {"gamma", 300}}};
    ...
    Json ref_test{Json::Obj{{"data1", 10}, {"data2", 20}}};
    ...
    Json assign_val{Json::Obj{}};
    assign_val = Json::Obj{{"key1", 1}, {"key2", 2}};
    M_ASSERT_EQ(assign_val.obj().size(), 2);

    // --- Nested Obj Access ---
    Json nested_obj{Json::Obj{
        {"person", Json::Obj{{"name", "John"}, {"age", 30}, {"active", true}}},
        {"company", Json::Obj{{"name", "TechCorp"}, {"employees", 100}, {"public", false}}}
    }};
    M_ASSERT_EQ(nested_obj["person"]["name"].to<Json::Str>(), "John");
    M_ASSERT_EQ(nested_obj["company"]["employees"].to<Json::Num>(), 100);

    // --- Deeply Nested Obj Access ---
    Json deep_nested{Json::Obj{
        {"level1", Json::Obj{
            {"level2", Json::Obj{
                {"level3", Json::Obj{
                    {"data1", "deep_value1"},
    ...
    Json obj_cmp1{Json::Obj{{"a", 1}, {"b", 2}}};
    Json obj_cmp2{Json::Obj{{"a", 1}, {"b", 2}}};
    Json obj_cmp3{Json::Obj{{"a", 1}, {"b", 3}}};
    M_ASSERT_TRUE(obj_cmp1 == obj_cmp2);
    M_ASSERT_FALSE(obj_cmp1 == obj_cmp3);

    // --- Serialization and Parsing ---
    Json serial_test{Json::Obj{{"key1", 1}, {"key2", 2}}};
    ...
    if (parsed_nested.has_value()) {
        M_ASSERT_EQ( parsed_nested->type(), json::Type::eObj );
        ...
        Json nested_code{Json::Obj{
            {"outer", Json::Obj{{"inner1", 1}, {"inner2", 2}}}
        }};
    ...
    if (parsed_mixed.has_value()) {
        M_ASSERT_EQ( parsed_mixed->type(), json::Type::eObj );
    ...
    Json original_obj{Json::Obj{
        {"level1", Json::Obj{
            {"name", "nested"},
            {"value", 123}
        }},
        {"level2", Json::Obj{
            {"items", Json::Obj{{"item1", 1}, {"item2", 2}}}
        }}
    }};
    ...
    // --- Contains Method ---
    Json contains_test{Json::Obj{{"key1", 1}, {"key2", "hello"}}};
    ...
    Json ref_modify_test{Json::Obj{{"a", 1}, {"b", 2}}};
