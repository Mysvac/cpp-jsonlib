...
M_TEST(ARR_MAP, ArrayTypes) {
    ...
    // std::vector<Json>
    const std::vector<Json> vec_val = {Json::Num(1), Json::Bol(false), Json::String("x")};
...
M_TEST(ARR_MAP, MapTypes) {
    ...
    const std::map<std::string, Json> map_val = {{"n", Json::Num(42)}, {"b", Json::Bol(true)}, {"s", Json::String("str")}};
...
M_TEST(ARR_MAP, UnorderedMapTypes) {
    ...
    const std::unordered_map<std::string, Json> umap_val = {{"num", Json::Num(7)}, {"flag", Json::Bol(false)}, {"txt", Json::String("abc")}};
